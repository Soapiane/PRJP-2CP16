...
class DatabaseRepository extends Repository {
  ...
  Future<void> getUserInfo() async {
    ...
    user.User().setDifficulty(Difficulty.values[prefs.getInt("difficulty") ?? 0]);
  }
  ...
  Future<void> uploadUserInfo() async {
    await FirebaseDatabase(databaseURL: databaseLink).reference().child("users").child(FirebaseAuth.instance.currentUser!.uid).update({
  ...
  Future<void> uploadZone(Map zone) async {
    await FirebaseDatabase(databaseURL: databaseLink).reference().child("users").child(FirebaseAuth.instance.currentUser!.uid).child("zones").child(zone["name"]).set({
      "id" : zone['id'],
      "name": zone['name'],
      "stars": zone['stars'],
  ...
  Future<void> uploadTrophy(Map trophy) async {
    await FirebaseDatabase(databaseURL: databaseLink).reference().child("users").child(FirebaseAuth.instance.currentUser!.uid).child("trophies").child(trophy["id"].toString()).update({
      "id" : trophy['id'],
  ...
  Future<void> download() async {
    ...
    await FirebaseDatabase(databaseURL: databaseLink).reference().child("users").child(FirebaseAuth.instance.currentUser!.uid).child("zones").get().then((snapshot) {
    ...
    await FirebaseDatabase(databaseURL: databaseLink).reference().child("users").child(FirebaseAuth.instance.currentUser!.uid).child("levels").onValue.listen((event) async {
    ...
    await FirebaseDatabase(databaseURL: databaseLink).reference().child("users").child(FirebaseAuth.instance.currentUser!.uid).child("trophies").onValue.listen((event) {
  ...
  Future<void> updateUserInfo() async {
    await FirebaseDatabase(databaseURL: databaseLink).reference().child("users").child(FirebaseAuth.instance.currentUser!.uid).once().then((DataSnapshot snapshot) {
      user.User().setName(snapshot.value["name"]);
      user.User().setAvatar(Avatar.values[snapshot.value["avatar"]]);
      user.User().setDifficulty(Difficulty.values[snapshot.value["difficulty"]]);
    });
  }
  
  Future<void> downloadAndUpdateZone(Zones zone) async {
    await FirebaseDatabase(databaseURL: databaseLink).reference().child("users").child(FirebaseAuth.instance.currentUser!.uid).child("zones").child(zone.toString().split(".")[1]).once().then((DataSnapshot snapshot) {
  ...
  Future<void> fetchAndDownloadTrophy(Trophies trophy) async {
    await FirebaseDatabase(databaseURL: databaseLink).reference().child("users").child(FirebaseAuth.instance.currentUser!.uid).child("trophies").child(trophy.id.toString()).once().then((DataSnapshot snapshot) {
